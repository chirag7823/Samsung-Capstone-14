...
// DigitalTwinApp.jsx
// Single-file React prototype for a Smart Factory Digital Twin frontend
// - Simulates devices (machines, lights, fans, coolers) with random power usage
// - Live updating floorplan + device cards
// - Aggregated power line chart (last N timesteps)
// - Controls: start/stop simulation, reset, export CSV, simulation speed, cost per kWh

export default function DigitalTwinApp() {
  // user-configurable values
  const TIMESTEPS_TO_KEEP = 40; // how many points to keep in the chart

  const [devices, setDevices] = useState(() => generateDevices(12));
  const [running, setRunning] = useState(true);
  const [history, setHistory] = useState([]);
  const [tickMs, setTickMs] = useState(1000);
  const [costPerKwh, setCostPerKwh] = useState(10); // currency per kWh (default: 10)
  const tickRef = useRef(null);

  // derived stats
  const totalPower = useMemo(() => devices.reduce((s, d) => s + d.power, 0), [devices]);
  const avgPower = useMemo(() => (devices.length ? Math.round(totalPower / devices.length) : 0), [totalPower, devices.length]);
  ...
  useEffect(() => {
    ...
    tickRef.current = setInterval(() => {
      ...
      setHistory((prev) => {
        const t = new Date();
        const newPoint = { time: t.toLocaleTimeString(), totalPower: Math.round(prevPowerFromDevices(devices)) };
  ...
  // keep history in sync with devices when user toggles quickly
  useEffect(() => {
    const t = new Date();
    setHistory((prev) => {
      const next = [...prev, { time: t.toLocaleTimeString(), totalPower }].slice(-TIMESTEPS_TO_KEEP);
      return next;
    });
  }, [totalPower]);
  ...
  function handleToggleDevice(id) {
    setDevices((prev) => prev.map((d) => (d.id === id ? { ...d, state: !d.state, power: !d.state ? randomPower(d.basePower) : 0 } : d)));
  }

  function handleExportCSV() {
    // export history + device snapshot
  ...
  function estimatedCostPerHour() {
    // totalPower is in Watts; convert to kW and multiply by costPerKwh
  ...
  return (
    <div className="p-6 font-sans text-slate-800">
      <header className="mb-4">
        <h1 className="text-2xl font-bold">Smart Factory — Digital Twin (Prototype)</h1>
        <p className="text-sm text-slate-600 mt-1">Live simulation of devices — generate synthetic data, inspect, export, and later connect to ML optimization.</p>
      </header>

      <div className="grid grid-cols-3 gap-4">
        {/* Floorplan / visual area */}
        <section className="col-span-2 bg-white rounded-2xl shadow p-4 relative h-[520px]">
          ...
          <div className="relative bg-slate-50 h-[440px] rounded-lg border border-dashed border-slate-200 overflow-hidden">
            ...
            <div className="absolute left-3 bottom-3 bg-white/90 rounded-md p-2 text-sm shadow">
              ...
              <div className="text-slate-500 text-xs">Est. ₹/hr: {estimatedCostPerHour()}</div>
            </div>
          </div>
        </section>
        ...
        <aside className="col-span-1 space-y-4">
          <div className="bg-white rounded-2xl shadow p-4">
            ...
            <div className="mt-3 flex gap-2">
              <button onClick={handleToggleRunning} className="px-3 py-2 rounded bg-indigo-600 text-white">{running ? "Pause" : "Start"}</button>
              <button onClick={handleReset} className="px-3 py-2 rounded border">Reset</button>
              <button onClick={handleExportCSV} className="px-3 py-2 rounded border">Export CSV</button>
            </div>

            <div className="mt-3">
              <label className="text-sm">Tick (ms)</label>
              <input type="range" min={200} max={2000} step={100} value={tickMs} onChange={(e) => setTickMs(Number(e.target.value))} />
            ...
            <div className="mt-3">
              ...
              <div className="flex items-center gap-2 mt-1">
                <input type="number" value={costPerKwh} onChange={(e) => setCostPerKwh(Number(e.target.value))} className="w-24 p-2 border rounded" />
                <div className="text-xs text-slate-500">Estimated cost/hr: ₹{estimatedCostPerHour()}</div>
              </div>
            </div>
          </div>
            ...
            <div style={{ width: "100%", height: 220 }} className="mt-2">
              <ResponsiveContainer>
                <LineChart data={history} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Line type="monotone" dataKey="totalPower" stroke="#2563eb" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <footer className="mt-6 text-sm text-slate-600">
        Next steps: connect this frontend to a backend (Flask/Node) that will gather the simulated telemetry into a dataset, train an ML model / optimization solver, and return an optimized operating schedule. I can implement the backend + dataset generator + baseline ML in Python (Flask + scikit-learn/XGBoost) — say the word and I will start the next step.
      </footer>
    </div>
  );
}
...
function prevPowerFromDevices(devices) {
  // aggregate power from given devices array (defensive copy)
  if (!devices || devices.length === 0) return 0;
  return devices.reduce((s, d) => s + (d.power ?? 0), 0);
}

function randomPower(base) {
  // base is in Watts; modulation factor simulates variable load
  const factor = 0.6 + Math.random() * 1.2; // 0.6x to 1.8x
  return Math.round(base * factor);
}

function randomUpdateDevice(d) {
  // small chance to toggle state on/off randomly
...
function generateDevices(n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const x = Math.round(Math.random() * 88) + 6; // keep inside 6-94%
    ...
    out.push({ id: `dev-${i + 1}`, name: `${t.type}-${i + 1}`, type: t.type, basePower, power, state, x, y });
  }
  return out;
}
